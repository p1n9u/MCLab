...
int BG96::esocketStart(char *buffer, int size) {
  char sendBuffer[16];
  char resBuffer[16];
  char *aLine[3]; // read up to 20 lines
...
int BG96::esocketSend(char *buffer, int size) {
  /* Send Data by Socket */
...
int BG96::esocketStop(char *buffer, int size) {
  /* Wait "SEND OK" */
  // BG96_serial_clearbuf();
...
int BG96::socketRecv(char *buffer, int bufferSize, int *recvSize,
  ...
  if (0 == sendServicecmd(szCmd, buffer, bufferSize, recvSize, _timeOut))
    return 0;
