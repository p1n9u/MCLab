...
int BG96::socketSend_mod(char *buffer, int size) {
  char sendBuffer[16];
  char resBuffer[16];
  char *aLine[3]; // read up to 20 lines
  ...
  i = readATresponseLine_mod(aLine, 3);
...
int BG96::socketSend_mod(const char *str) {
  return socketSend((char *)str, strlen(str));
}

int BG96::readATresponseLine_mod(char *aLine[], int nMaxLine) {
  int nbLine = 0;
  char *pszSubstr = NULL;
  int i;
